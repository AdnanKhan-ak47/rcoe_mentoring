...
const ProblemItem = (props) => {
    let { title, description, date_start, id, is_solved } = props
    const context = useContext(ProblemContext)
    const { setUpdate } = context

    const deleteProblem = async (id) => {
        const response = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/delete_problem/${id}`, {
            method: 'DELETE',
            headers: {
    ...
    return (
        <div className='border border-gray-200 shadow-md m-5 rounded-lg p-2 flex flex-row justify-between items-center'>
            <div className='flex flex-col border '>
                <div className=''>
                    {is_solved 
                    ? (<span className="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full ">Resolved</span>) 
                ...
                <div className=''>
                    ...
                    <div className='font-mono text-xs'>
                        By <Link href={'#'}><span className='hover:underline'>Mentee</span></Link> On <span>{date_start}</span>
                    </div>
                </div>
            </div>
            <div onClick={() => { deleteProblem(id) }} className='flex place-self-end border border-slate-200 hover:bg-slate-400 self-center'>
                <svg className='' xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="50" height="50" viewBox="0 0 100 100">
